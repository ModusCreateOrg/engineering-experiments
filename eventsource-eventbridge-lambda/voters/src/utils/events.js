 ...
 */
exports.sendBallotVerifiedEvent = async (detail = {}) => {
  console.log('Events::sendBallotVerifiedEvent');

  const config = {
    url: `${baseUrl}/events`,
    method: 'post',
    data: {
      type: 'Ballot Verified',
      detail,
    },
  };
...
/**
 * Send a 'Ballot Rejected' event.
 * @param {Object} detail An object containing the event detail (i.e. payload).
 * @returns An object containing the event confirmation.
 * @throws An Error if all attempts to send the event fail.
 */
exports.sendBallotRejectedEvent = async (detail = {}) => {
  console.log('Events::sendBallotRejectedEvent');

  const config = {
    url: `${baseUrl}/events`,
    method: 'post',
    data: {
      type: 'Ballot Rejected',
      detail,
    },
  };
